...
class PlaybackOverlayFragment : PlaybackOverlayFragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        onItemViewClickedListener = OnItemViewClickedListener { itemViewHolder, item, rowViewHolder, row -> Log.i(TAG, "onItemClicked: $item row $row") }
    }
    ...
    protected fun updateVideoImage(uri: String) {
        Glide.with(activity)
                .load(uri)
                .centerCrop()
                .into(object : SimpleTarget<GlideDrawable>(CARD_WIDTH, CARD_HEIGHT) {
